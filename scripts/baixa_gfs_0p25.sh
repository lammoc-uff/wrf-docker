...
# Baixa os arquivos até 96 horas, de 3 em 3.

echo ""$dia"/"$mes"/"$ano""

    for i in  `seq 0 3 96`; do
        hora=`printf "%03i\n" $i`
        echo $HORA >> $RELATORIOGFSFTP
        echo $hora >> $RELATORIOGFSFTP
        $WGET -a -O "gfs.t00z.pgrb2.0p25.f${hora}" -nc "https://nomads.ncep.noaa.gov/cgi-bin/filter_gfs_0p25.pl?file=gfs.t00z.pgrb2.0p25.f${hora}&all_lev=on&all_var=on&subregion=&leftlon=280&rightlon=350&toplat=10&bottomlat=-55&dir=%2Fgfs.${ano}${mes}${dia}%2F00%2Fatmos"
